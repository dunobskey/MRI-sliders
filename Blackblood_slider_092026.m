...
box_tr = uieditfield(fig, 'numeric', 'Position', [115 135 60 25], 'Value', 0.7, 'FontSize', 14, 'FontWeight', 'bold', 'Limits', [0 2]);
sld_tr = uislider(fig, 'Position', [200 150 280 3], 'Limits', [0 2], 'Value', 0.7, 'FontSize', 16, 'FontWeight', 'bold');
sld_tr.MajorTicks=[0 0.5 1.0 1.5 2];

% 3. Create TE Controls
uilabel(fig, 'Text', 'TE (ms):', 'FontSize', 15, 'Position', [40 85 70 22], 'FontWeight', 'bold');
box_te = uieditfield(fig, 'numeric', 'Position', [115 80 60 25], 'Value', 35, 'FontSize', 14, 'FontWeight', 'bold', 'Limits', [10 250]);
sld_te = uislider(fig, 'Position', [200 95 280 3], 'Limits', [10 250], 'Value', 35, 'FontSize', 16, 'FontWeight', 'bold');

% 4. Create FA Controls
uilabel(fig, 'Text', 'Flip angle:', 'FontSize', 15, 'Position', [40 30 90 22], 'FontWeight', 'bold');
box_fa = uieditfield(fig, 'numeric', 'Position', [125 25 50 25], 'Value', 80, 'FontSize', 14, 'FontWeight', 'bold', 'Limits', [0 90]);
sld_fa = uislider(fig, 'Position', [200 40 280 3], 'Limits', [0 90], 'Value', 80, 'FontSize', 16, 'FontWeight', 'bold');
...
function draw(ax, sliders)
    ...
    title(ax, sprintf('Blackblood | TR: %.0f ms | TE: %.0f ms | FA: %.0f deg', t_r, t_e, f_a));
