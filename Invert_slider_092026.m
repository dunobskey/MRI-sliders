...
box_tr = uieditfield(fig, 'numeric', 'Position', [115 135 60 25], 'Value', 15, 'FontSize', 14, 'FontWeight', 'bold', 'Limits', [0 25]);
sld_tr = uislider(fig, 'Position', [200 150 280 3], 'Limits', [0 25], 'Value', 15, 'FontSize', 16, 'FontWeight', 'bold');
sld_tr.MajorTicks = [0,5,10,15,20,25];

% 3. Create TE Controls
uilabel(fig, 'Text', 'TE (ms):', 'FontSize', 15, 'Position', [40 85 70 22], 'FontWeight', 'bold');
box_te = uieditfield(fig, 'numeric', 'Position', [115 80 60 25], 'Value', 580, 'FontSize', 14, 'FontWeight', 'bold', 'Limits', [100 1000]);
sld_te = uislider(fig, 'Position', [200 95 280 3], 'Limits', [100 1000], 'Value', 580, 'FontSize', 16, 'FontWeight', 'bold');
...
function draw(ax, sliders)
    ...
    j = [4035, 1800, 1300, 1000; 1885, 150, 100, 80; "CSF", "Blood", "Gray matter", "White matter"];
    R_1relaxivity = 4.71; R_2relaxivity = 5.47;
    C = linspace(0, 1.6, 201)'; % Static contrast baseline range
    
    cla(ax);
    hold(ax, 'on');
    t_i=str2double(j(1,1))*log(2/(1+exp(-t_r/4035)));
    ...
    title(ax, sprintf('Heavily T2-weighted FLAIR | TR: %.0f ms | TE: %.0f ms | TI: %.0f ms', t_r, t_e, t_i));
